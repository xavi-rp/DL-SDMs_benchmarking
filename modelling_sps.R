...
#write.csv(worldclim_all_data, paste0(preds_dir, "worldclim_all_data_ibp.csv"), row.names = FALSE)
...
write.csv(bckgr, paste0(preds_dir, "background_points_ibp.csv"), row.names = FALSE)
...
virt_sp_dir <- "/eos/jeodpp/home/users/rotllxa/DL-SDM_benchmarking_data/virtualspecies_ibp/"
...
for (t in taxons){
  ...
  model_ct %>% 
    ...
    #layer_conv_2d(filters = 9, kernel_size = c(2, 2), data_format = "channels_last", padding = "valid", activation = "relu") %>%
    #
    #layer_dropout(0.25) %>%
    layer_flatten() %>%
  ...
  if(!file.exists(paste0(preds_dir, "worldclim_all_tensor_array_ibp.rds"))){
    ...
    worldclim_all_tensor_array <- readRDS(paste0(preds_dir, "worldclim_all_tensor_array_ibp.rds"))
  }
